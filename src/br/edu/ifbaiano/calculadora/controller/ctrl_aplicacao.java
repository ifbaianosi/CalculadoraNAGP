...
//		int qtdPassagem = Integer.parseInt(getJTextField_qtdPassagem().getText());
		...
		/**
		 * setando os valores no campos do formul�rio
		 */
	...
	private void calcula_porMes() {
		...
		/**
		 * calculo do 6% de desconto do vencimento b�sico
		 */
	...
	private void imprimir_porMes() {
		try {
			...
			JOptionPane.showMessageDialog(rootPane, getJTabbedPane().getTitleAt(0)+".pdf (O arquivo j� est� aberto.)");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void imprimir_porDia() {
		try {
			...
			JOptionPane.showMessageDialog(rootPane, getJTabbedPane().getTitleAt(1)+".pdf (O arquivo j� est� aberto.)");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
